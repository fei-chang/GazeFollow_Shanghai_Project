...
"""
Please note that this script is a adopted from codes for paper 'Detecting Attended Visual Targets in Video'
Original codes can be found in github https://github.com/ejcgt/attention-target-detection
"""
...
def get_head_box_channel(x_min, y_min, x_max, y_max, width, height, input_resolution, coordconv=False):

    head_box = np.array([x_min/width, y_min/height, x_max/width, y_max/height])*input_resolution
    head_box = head_box.astype(int)
    head_box = np.clip(head_box, 0, input_resolution-1)
    if coordconv:
        unit = np.array(range(0, input_resolution), dtype=np.float32)
        head_channel = []
        for i in unit:
        ...
        head_channel = np.zeros((input_resolution, input_resolution), dtype=np.float32)
